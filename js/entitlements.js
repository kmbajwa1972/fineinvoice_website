...
async function loadUser(){
  let auth=null;
  try{auth=typeof getAuthenticatedUser==='function'?await getAuthenticatedUser():null}catch(e){console.warn('FineInvoice auth read failed:',e)}
  let u=auth?.user||(typeof getAuthUser==='function'?await getAuthUser():getCurrentUser?.());
  if(!u)return null;
  const meta=u.user_metadata||{}, prev=getCurrentUser()||{};
  const plan=String(meta.plan??u.plan??prev.plan??'free').toLowerCase();
  u={...prev,id:u.id,email:u.email||prev.email||'',name:meta.name||prev.name||u.email||'User',
    plan,planVerified:meta.planVerified===true,paymentProvider:meta.paymentProvider||prev.paymentProvider||null,
    ...
    singleCredits:0,
    unlockedInvoiceIds:Array.isArray(meta.unlockedInvoiceIds)?[...new Set(meta.unlockedInvoiceIds.map(String))]:(prev.unlockedInvoiceIds||[])};
...
async function consumePaidCredit(u,id){
  ...
  const ids=[...new Set([...(u.unlockedInvoiceIds||[]).map(String),String(id)])],next=paid-1;
  const sb=typeof getSupabase==='function'?getSupabase():null;
  if(sb){
    const {error:ae}=await sb.auth.updateUser({data:{paidSingleCredits:next,unlockedInvoiceIds:ids,singleCredits:Number(u.freePdfCredits||0)+next}});
    if(ae)return{ok:false,error:ae.message||'Could not save PDF entitlement.'};
    if(u.id){try{await sb.from('profiles').update({single_credits:next}).eq('id',u.id)}catch(e){}}
  }
  u.paidSingleCredits=next;u.singleCredits=Number(u.freePdfCredits||0)+next;u.unlockedInvoiceIds=ids;saveCurrentUser(u);
...
function styles(){
  ...
  if(!document.getElementById('fineinvoice-print-dashboard')){
    const b=document.createElement('button');b.id='fineinvoice-print-dashboard';b.type='button';b.textContent='← Dashboard';b.onclick=()=>{document.body.classList.remove('print-mode');location.href='dashboard.html'};document.body.appendChild(b)
  }
}
...
async function makePdf(){
  ...
  try{
    const c=await html2canvas(p,{scale:3,useCORS:true,backgroundColor:'#fff',scrollX:0,scrollY:0,logging:false,windowWidth:Math.max(document.documentElement.clientWidth,p.scrollWidth)});
    const {jsPDF}=window.jspdf,W=210,M=6,CW=W-M*2,CH=297-M*2,mm=CW/c.width,total=c.height*mm,pages=Math.max(1,Math.ceil(total/CH)),doc=new jsPDF({orientation:'portrait',unit:'mm',format:'a4'});
    for(let pg=0;pg<pages;pg++){if(pg)doc.addPage('a4','portrait');const sy=Math.floor(pg*CH/mm),sh=Math.min(c.height-sy,Math.ceil(CH/mm));if(sh<=0)continue;const slice=document.createElement('canvas');slice.width=c.width;slice.height=sh;const x=slice.getContext('2d');x.fillStyle='#fff';x.fillRect(0,0,slice.width,slice.height);x.drawImage(c,0,sy,c.width,sh,0,0,c.width,sh);doc.addImage(slice.toDataURL('image/jpeg',.96),'JPEG',M,M,CW,sh*mm)}
    return doc
  }finally{el.classList.remove('pdf-capture')}
}

function installSaveRepair(){
  if(window.__fineInvoiceSaveRepairInstalled)return;
  window.__fineInvoiceSaveRepairInstalled=true;
  const original=window.saveInvoice;
  if(typeof original!=='function')return;
  window.saveInvoice=function(){try{return original.apply(this,arguments)}catch(e){console.error('FineInvoice save failed:',e);showToast('Invoice could not be saved. Please try again.','error',5000)}}
}
...
window.addEventListener('load',()=>{
  ...
  setTimeout(()=>{
    ...
    window.downloadPDF=async function(){
      ...
      if(!u){showToast('Please sign in again to create a PDF.','error',5000);return}
      const n=invoiceId(),life=String(u.plan||'free').toLowerCase()==='lifetime',open=unlocked(u),free=Math.max(0,Number(u.freePdfCredits||0)),paid=Math.max(0,Number(u.paidSingleCredits||0));
      if(!life&&!open&&invNo()>4&&free+paid<=0){showToast('No PDF credits remaining. Please purchase a PDF credit or Lifetime Access.','error',5000);if(confirm('You have no PDF credits remaining.\n\nSingle: $2 per PDF\nor Lifetime: $25 unlimited.\n\nGo to Billing?'))location.href='payment.html';return}
      showToast('Generating PDF…','info');
      try{
        const doc=await makePdf();
        if(!life&&!open&&invNo()>4){const r=await consumePaidCredit(u,n);if(!r.ok)throw Error(r.error)}
      ...
      }catch(e){console.error('FineInvoice PDF generation failed:',e);showToast('PDF generation failed. Your PDF credit was not used.','error',5000)}
    };
    window.printInvoice=async function(){
      const u=await loadUser();
      if(!u){showToast('Please sign in again to print.','error',5000);return}
      const n=invoiceId(),life=String(u.plan||'free').toLowerCase()==='lifetime',open=unlocked(u),free=Math.max(0,Number(u.freePdfCredits||0)),paid=Math.max(0,Number(u.paidSingleCredits||0));
      if(!life&&!open&&invNo()>4&&free+paid<=0){showToast('No PDF credits remaining. Please purchase a PDF credit or Lifetime Access.','error',5000);return}
      if(!life&&!open&&invNo()>4){const r=await consumePaidCredit(u,n);if(!r.ok){showToast(r.error,'error',5000);return}}
      document.body.classList.add('print-mode');window.print();setTimeout(()=>document.body.classList.remove('print-mode'),800)
    };
  },0)
});
})();