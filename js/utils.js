...
function normalizePlanUser(user) {
  ...
  const verified = user.planVerified === true;
  if ((plan === 'single' || plan === 'lifetime') && !verified) plan = 'free';
  user.plan = plan;
  user.planVerified = verified;
  const hasExplicitCreditFields = Object.prototype.hasOwnProperty.call(user, 'freePdfCredits') || Object.prototype.hasOwnProperty.call(user, 'paidSingleCredits');
  user.freePdfCredits = Number.isFinite(Number(user.freePdfCredits)) ? Math.max(0, Number(user.freePdfCredits)) : (hasExplicitCreditFields ? 0 : 3);
...
function applyRemoteEntitlementUser(remoteUser) {
  ...
  const rawPlan = String(meta.plan || previous.plan || 'free').toLowerCase();
  const verified = meta.planVerified === true;
  const plan = verified && ['single','lifetime'].includes(rawPlan) ? rawPlan : 'free';
  const freeRaw = meta.freePdfCredits ?? previous.freePdfCredits ?? 3;
  ...
  const next = {...previous,id:remoteUser.id,email:remoteUser.email||previous.email||'',name:meta.name||previous.name||remoteUser.email||'User',plan,planVerified:verified,paymentProvider:meta.paymentProvider||previous.paymentProvider||null,plan_activated_at:meta.plan_activated_at||previous.plan_activated_at||null,freePdfCredits,paidSingleCredits,singleCredits:freePdfCredits+paidSingleCredits,unlockedInvoiceIds:Array.isArray(meta.unlockedInvoiceIds)?[...new Set(meta.unlockedInvoiceIds.map(String))]:(previous.unlockedInvoiceIds||[]),whatsapp:meta.whatsapp||previous.whatsapp||null};
  const changed=String(previous.plan||'free')!==plan||Boolean(previous.planVerified)!==verified||Number(previous.freePdfCredits??3)!==freePdfCredits||Number(previous.paidSingleCredits||0)!==paidSingleCredits||JSON.stringify(previous.unlockedInvoiceIds||[])!==JSON.stringify(next.unlockedInvoiceIds||[]);
...
window.addEventListener('DOMContentLoaded',()=>{const originalPrint=window.printInvoice;async function loadGateUser(){const auth=await getAuthenticatedUser();if(!auth.user)return{user:null,error:auth.error};const a=auth.user,m=a.user_metadata||{},previous=getCurrentUser()||{},verified=m.planVerified===true,rawPlan=String(m.plan||previous.plan||'free').toLowerCase(),plan=verified&&['single','lifetime'].includes(rawPlan)?rawPlan:'free';const user={...previous,id:a.id,email:a.email||previous.email||'',name:m.name||previous.name||a.email||'User',plan,planVerified:verified,paymentProvider:m.paymentProvider||previous.paymentProvider||null,freePdfCredits:Math.max(0,Number(m.freePdfCredits??previous.freePdfCredits??3)||0),paidSingleCredits:Math.max(0,Number(m.paidSingleCredits??previous.paidSingleCredits??0)||0),unlockedInvoiceIds:Array.isArray(m.unlockedInvoiceIds)?m.unlockedInvoiceIds:(previous.unlockedInvoiceIds||[])};user.singleCredits=user.freePdfCredits+user.paidSingleCredits;saveCurrentUser(user);return{user,error:null};}
...
window.addEventListener('load',()=>{if(window.__fineInvoiceStrictPdfGateWrapped)return;const productionPDF=window.downloadPDF;if(typeof productionPDF!=='function')return;window.__fineInvoiceStrictPdfGateWrapped=true;window.downloadPDF=async function(){const auth=await getAuthenticatedUser();if(!auth.user){showToast(auth.error||'Please sign in again.','error',5000);return;}const remote=auth.user.user_metadata||{},previous=getCurrentUser()||{},user=normalizePlanUser({...previous,id:auth.user.id,email:auth.user.email||previous.email||'',name:remote.name||previous.name||auth.user.email||'User',plan:remote.plan||previous.plan||'free',planVerified:remote.planVerified===true,freePdfCredits:remote.freePdfCredits??previous.freePdfCredits??3,paidSingleCredits:remote.paidSingleCredits??previous.paidSingleCredits??0,unlockedInvoiceIds:remote.unlockedInvoiceIds??previous.unlockedInvoiceIds??[]});saveCurrentUser(user);const invoiceId=typeof currentDraftId!=='undefined'?String(currentDraftId):(document.getElementById('invNumber')?.value||'draft'),plan=String(user.plan||'free').toLowerCase(),unlocked=Array.isArray(user.unlockedInvoiceIds)&&user.unlockedInvoiceIds.map(String).includes(invoiceId);if(plan!=='lifetime'&&!unlocked&&Number(user.freePdfCredits||0)<=0&&Number(user.paidSingleCredits||0)<=0){showToast('No PDF credits remaining. Please purchase a PDF credit or Lifetime Access.','error',5000);if(confirm('You have no PDF credits remaining.\n\nSingle: $2 per PDF\nor Lifetime: $25 unlimited.\n\nGo to Billing?'))location.href='payment.html';return;}if(plan==='lifetime'||unlocked){await productionPDF();return;}const beforeFree=Number(user.freePdfCredits||0),beforePaid=Number(user.paidSingleCredits||0),beforeDownloads=parseInt(localStorage.getItem('fi_downloads')||'0',10);const consumed=await consumeInvoiceCredit(user,invoiceId);if(!consumed.ok){showToast(consumed.error||'No PDF credits remaining.','error',5000);return;}try{await productionPDF();const afterDownloads=parseInt(localStorage.getItem('fi_downloads')||'0',10);if(afterDownloads<=beforeDownloads){const sb=getSupabase();if(sb){const refundMeta={freePdfCredits:beforeFree,paidSingleCredits:beforePaid,singleCredits:beforeFree+beforePaid,unlockedInvoiceIds:(user.unlockedInvoiceIds||[]).filter(x=>String(x)!==String(invoiceId))};await sb.auth.updateUser({data:refundMeta});user.freePdfCredits=beforeFree;user.paidSingleCredits=beforePaid;user.singleCredits=beforeFree+beforePaid;user.unlockedInvoiceIds=refundMeta.unlockedInvoiceIds;saveCurrentUser(user);}showToast('PDF could not be generated. Your credit was restored.','error',5000);return;}console.log('FineInvoice PDF credit consumed:',{beforeFree,beforePaid,afterFree:consumed.freePdfCredits,afterPaid:consumed.paidSingleCredits});refreshEntitlementUI(false);}catch(error){const sb=getSupabase();if(sb){const refundMeta={freePdfCredits:beforeFree,paidSingleCredits:beforePaid,singleCredits:beforeFree+beforePaid,unlockedInvoiceIds:(user.unlockedInvoiceIds||[]).filter(x=>String(x)!==String(invoiceId))};await sb.auth.updateUser({data:refundMeta});user.freePdfCredits=beforeFree;user.paidSingleCredits=beforePaid;user.singleCredits=beforeFree+beforePaid;user.unlockedInvoiceIds=refundMeta.unlockedInvoiceIds;saveCurrentUser(user);}console.error('FineInvoice PDF generation failed:',error);showToast('PDF could not be generated. Your credit was restored.','error',5000);}};});
